...
while [ $# -gt 0 ]; do
	...
	-f)
		if [ "x$2" = "x$QFIXFLAG" ] || [ "x$2" = "x$QPKGFLAG" ]; then
			BUILDQNAPFILE=1
			[ "x$2" = "x$QFIXFLAG" ] && QNAPFILE="${QPKG_NAME}.qfix"
  	fi
		_fill_up_str $2
...
if [ $BUILDQNAPFILE -eq 1 ];then
	...
	FINAL_STR="${QNAPMODEL_STR}${RESERVED_SPACE}${QNAPNAME_STR}${QNAPVER_STR}${QNAPFLAG_STR}"
fi
